...
export class ConfigurationService {
  ...
  constructor(private configService: ConfigService) {
    ...
    const envVariables = {
      ...
      AI_MODELS_CONFIG_PATH: this.configService.get<string>('AI_MODELS_CONFIG_PATH'),
    ...
    this.modelsConfigPath = path.resolve(process.cwd(), this.config.AI_MODELS_CONFIG_PATH);
    try {
      const modelsFileContent = fs.readFileSync(this.modelsConfigPath, 'utf-8');
      this.modelsConfig = JSON.parse(modelsFileContent);
      this.logger.log(`✅ Models configuration loaded from ${this.modelsConfigPath}`);
    } catch (error) {
      this.logger.error(`❌ Failed to load models configuration from ${this.modelsConfigPath}:`, error);
  ...
  private getApiKeyForProvider(provider: string): string {
    ...
    if (!apiKey) {
        throw new Error(`API key for provider "${provider}" is not configured in environment variables.`);
    }
    return apiKey;
  }
  ...
  get ai() {
    ...
    const selectedModelConfig: ModelLevelConfig | undefined = this.modelsConfig[modelLevel];

    if (!selectedModelConfig) {
      const availableLevels = Object.keys(this.modelsConfig).join(', ');
      throw new Error(`Invalid AI_MODEL_LEVEL: "${modelLevel}". Available levels: ${availableLevels}`);
    }
