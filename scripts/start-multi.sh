...
# Global variables
declare SELECTED_AGENT_CONFIG="default.multi-agent.json"
...
run_interactive_command() {
  ...
  lerna run --scope @starknet-agent-kit/agents multi-agent -- --agent="${SELECTED_AGENT_CONFIG}" || return $?
...
select_agent_config() {
  ...
  echo -e "\n${YELLOW}Enter the name of the Agent configuration to use (without .multi-agent.json extension):${NC}"
  ...
  if [ -z "$input" ]; then
    ...
    SELECTED_AGENT_CONFIG="default.multi-agent.json"
