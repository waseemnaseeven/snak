...
const mockedGetAllRawPrograms = getAllRawPrograms as jest.MockedFunction<
  typeof getAllRawPrograms
>;
...
describe('fixCairoCode', () => {
  ...
  it('should successfully fix Cairo code and update it in the database', async () => {
    ...
    // Call the function
    const result = await fixCairoCode(mockAgent as any, {
    ...
    expect(parsedResult.fixedCode).toBe(
      'fn main() {\n    // Fixed Cairo code\n}'
    );
    expect(parsedResult.originalCode).toBe(
      'fn main() {\n    // Buggy Cairo code\n}'
    );
        ...
        messages: expect.arrayContaining([
          expect.objectContaining({
            ...
            content: expect.stringContaining(
              'The error is: There is a bug in the main function'
            ),
          }),
        ]),
      }),
