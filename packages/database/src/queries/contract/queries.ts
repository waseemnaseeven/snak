...
export namespace contract {
	 ...
	 */
	export type Contract<HasId extends Id = Id.NoId> =
		HasId extends Id.Id ? ContractWithId : ContractBase;
	 ...
	 * @throws { DatabaseError } If a database operation fails.
	 */
	export async function selectContract(classHash: string): Promise<Contract<Id.Id> | undefined> {
		const q = new Query(
			`SELECT 
				id, 
				class_hash, 
	 ...
	 * @field { string } [deploy_tx_hash] - Hash of the deploy transaction.
	 * 
	 * @throws { DatabaseError } If a database operation fails.
	 */
	export type Deployment<HasId extends Id = Id.NoId> =
		HasId extends Id.Id ? DeploymentWithId : DeploymentBase;
