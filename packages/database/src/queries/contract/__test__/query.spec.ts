...
describe('Deployment table', () => {
	it('Should handle insertions', async () => {
		...
		await expect(contract.insertDeployment(deployment1, class_hash1)).resolves.toBeUndefined();
		...
		await expect(contract.insertDeployment(deployment2, class_hash2)).resolves.toBeUndefined();
	});
	...
	it('Should handle retrievals', async () => {
		...
		await expect(contract.selectDeployment(contract_address)).resolves.toMatchObject(deployment);
	});
	...
	it('Should handle bulk retrievals', async () => {
		...
		await expect(contract.selectDeployments(class_hash)).resolves.toMatchObject(deployments);
	})
